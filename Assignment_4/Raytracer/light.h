...
public:
    ...
    // VIRTUAL METHOD
    virtual void getIllumination(const Vec3f& p, Vec3f& dir, Vec3f& col, float &distanceToLight) const
    {
...
public:
    ...
    PointLight(const Vec3f& p, const Vec3f& c, float a1, float a2, float a3)
        : position(p), color(c), attenuation_1(a1), attenuation_2(a2), attenuation_3(a3) {}
    ~PointLight() {}
    ...
    virtual void getIllumination(const Vec3f& p, Vec3f& dir, Vec3f& col, float& distanceToLight) const 
    {
        ...
        attenuation = fmaxf(attenuation, 0);
