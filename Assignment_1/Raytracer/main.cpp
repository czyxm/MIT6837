#include <iostream>
#include "scene_parser.h"
#include "group.h" 
#include "material.h"
#include "camera.h" 
#include "ray.h"
#include "hit.h"
#include "image.h"

using namespace std;

float clamp(float value, float low, float high)
{
	assert(low <= high);
	if (value <= low)
	{
		return 0;
	}
	else if (value >= high)
	{
		return 1;
	}
	else
	{
		return (value - low) / (high - low);
	}
}

int main(int argc, char** argv)
{
	// ========================================================
	// ========================================================
	// Some sample code you might like to use for parsing 
	// command line arguments 
	
	char* input_file = NULL;
	int width = 100;
	int height = 100;
	char* output_file = NULL;
	float depth_min = 0;
	float depth_max = 1;
	char* depth_file = NULL;
	
	// sample command line:
	// raytracer -input scene1_1.txt -size 200 200 -output output1_1.tga -depth 9 10 depth1_1.tga
	
	for (int i = 1; i < argc; i++) {
	    if (!strcmp(argv[i], "-input")) {
	        i++; assert(i < argc);
	        input_file = argv[i];
	    }
	    else if (!strcmp(argv[i], "-size")) {
	        i++; assert(i < argc);
	        width = atoi(argv[i]);
	        i++; assert(i < argc);
	        height = atoi(argv[i]);
	    }
	    else if (!strcmp(argv[i], "-output")) 
		{
	        i++; assert(i < argc);
	        output_file = argv[i];
	    }
	    else if (!strcmp(argv[i], "-depth")) 
		{
	        i++; assert(i < argc);
	        depth_min = atof(argv[i]);
	        i++; assert(i < argc);
	        depth_max = atof(argv[i]);
	        i++; assert(i < argc);
	        depth_file = argv[i];
	    }
	    else 
		{
	        printf("whoops error with command line argument %d: '%s'\n", i, argv[i]);
	        assert(0);
	    }
	}
	
	// ========================================================
	// ========================================================
	// parse scene
	float halfWidth = width / 2.f, halfHeight = height / 2.f;
	Image renderImg(width, height);
	Image depthImg(width, height);
	SceneParser scene(input_file);
	OrthographicCamera* camera = dynamic_cast<OrthographicCamera*>(scene.getCamera());
	Group* group = scene.getGroup();
	renderImg.SetAllPixels(scene.getBackgroundColor());
	depthImg.SetAllPixels(scene.getBackgroundColor());
	for (int i = 0; i < width; i++)
	{
		for (int j = 0; j < height; j++)
		{
			Vec2f pos((i - halfWidth) * camera->getSize() / width, (j - halfHeight) * camera->getSize() / height);
			Ray ray = camera->generateRay(pos);
			Hit hit(camera->getTMin(), nullptr);
			if (group->intersect(ray, hit, camera->getTMin()) && hit.getMaterial() != nullptr)
			{
				// render mode
				renderImg.SetPixel(i, j, hit.getMaterial()->getDiffuseColor());
				// depth mode
				float depth = 1 - clamp(hit.getT(), depth_min, depth_max);
				depthImg.SetPixel(i, j, Vec3f(depth, depth, depth));
			}
		}
	}
	renderImg.SaveTGA(output_file);
	depthImg.SaveTGA(depth_file);

	cout << "FINISHED!" << endl;

	return 0;
}